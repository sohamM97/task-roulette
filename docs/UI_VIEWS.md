...
- **Expanded dialog** — full-screen dialog opened by tapping a starred card with children. Shows task tree with lazy-expanding nodes. Chevron (`>` / `v`) on non-leaf nodes to expand/collapse. Leaf nodes show `open_in_new` icon. Same priority highlighting, blocked dimming, and dependency-chain ordering as tree preview via shared `childTextStyle()` helper and `reorderByDependencyChains()`.
  - **Header row icons** (left → right): coloured accent bar, star icon (tap → unstar confirmation), task name, **add subtask** (`+`), **go to task** (`open_in_new`).
  - **Add subtask button** (`+`) — opens the standard Add Task dialog (no pin/inbox toggles). Creates the new task as a direct child of the starred task and refreshes the tree in place. If the starred task is pinned in Today's 5, the **"This task is pinned"** warning dialog appears first (same wording as the All Tasks add flow); confirming transfers the Today's 5 pin to the new subtask so the pin indicator doesn't silently disappear when the parent becomes non-leaf.
