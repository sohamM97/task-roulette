...
- **Always run `flutter analyze` before committing** and fix any issues. CI runs `flutter analyze` with `--fatal-infos`, so even `info`-level diagnostics (e.g. deprecated API usage) will fail the build.
