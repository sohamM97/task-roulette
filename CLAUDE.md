...
- **Text in Row widgets:** Any `Text` with dynamic/user-generated content (task names, dependency names, etc.) inside a `Row` **must** be wrapped in `Expanded` or `Flexible`. Without this, `TextOverflow.ellipsis` and `maxLines` have no effect — the text overflows the row and causes a `RenderFlex overflowed` error. Static short labels (e.g. "Pin", "Inbox") are fine without wrapping.
- **Shared utilities:** `display_utils.dart` has `showInfoSnackBar(context, message, {onUndo})` — use this for all snackbars (includes close icon, undo support, proper duration). Don't create raw `SnackBar` objects. Also has DRY icon constants (`deadlineIcon`, `scheduledTodayIcon`, `archiveIcon`, `spinIcon`) — use these instead of hardcoding `Icons.*` for shared icons.
- **Starred view DRY rule:** The starred screen has two views of the same data — tree preview (card) and expanded dialog. **Any visual treatment (priority highlighting, blocked dimming, etc.) must apply to both views via a shared helper.** Currently `childTextStyle()` in `starred_screen.dart` handles priority + blocked styling for both.
- **Reuse existing dialogs/flows — don't reimplement.** Before building any dialog, picker, or multi-step interaction (add/edit/schedule/triage/brain-dump, etc.), first check whether the same thing already exists elsewhere. Read `docs/UI_VIEWS.md` (Dialogs section) and grep `lib/widgets/` to find it. If it exists, **reuse or extract a shared component** rather than copy-pasting the flow into a new screen — parameterize the parts that differ (parenting, which toggles to show, callbacks) instead of duplicating logic. Example: the "add task / add multiple (brain dump)" flow — including the Today's 5 pinned-parent warning and pin transfer — is the shared `AddTaskFlow` (`lib/widgets/add_task_flow.dart`), used by both All Tasks and the Starred expanded dialog; a third caller should reuse it, not re-create it.
- **Today's 5 weighted selection:** Every call to `pickWeightedN()` in `todays_five_screen.dart` must pass `scheduleBoostedIds`, `deadlineDaysMap`, and `normData` — use `_fetchSelectionContext()` to obtain them. Omitting these silently drops the schedule boost (2.5x), deadline boost (up to 8x), and normalization. The only exception is `pickWeightedN` calls that intentionally operate on a pre-filtered subset where these don't apply.
