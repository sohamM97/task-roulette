...
class TodaysFiveScreenState extends State<TodaysFiveScreen>
  ...
  /// weighted selection as the All Tasks roulette. A nudge, not the list — the
  /// user chooses whether to accept one. Stable across refreshes (no reshuffle);
  /// only consumed/ineligible entries are replaced, backfilled to
  /// [_suggestionTarget]. See [_refreshSuggestions].
  List<Task> _suggestions = [];
  /// Suggestions the user dismissed this session ("Not now") — excluded from
  /// re-selection so a dismissed task doesn't immediately reappear.
  final Set<int> _dismissedSuggestionIds = {};
  /// How many suggestions to surface at a time (user spec: 3-4).
  static const int _suggestionTarget = 4;
  ...
  ///  - Existing suggestions stay put (no reshuffle on every refresh); only
  ///    entries that were accepted, completed, blocked, or otherwise became
  ///    ineligible are pruned.
  ///  - Backfills fresh picks up to [_suggestionTarget], excluding tasks in
  ///    Today's 5, done today, already suggested, or dismissed this session.
  Future<void> _refreshSuggestions() async {
    ...
    // Already at target after prune → keep as-is (avoid per-refresh DB churn).
    if (_suggestions.length >= _suggestionTarget) return;
    ...
    final needed = _suggestionTarget - _suggestions.length;
    final picked = provider.pickWeightedN(
      pool,
      needed,
    ...
    _suggestions = [..._suggestions, ...picked];
  }
