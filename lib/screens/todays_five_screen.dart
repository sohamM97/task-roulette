...
class TodaysFiveScreenState extends State<TodaysFiveScreen>
  ...
  /// user chooses whether to accept one. Holds up to [_suggestionCap] eligible
  /// leaves in weighted order (best first), picked in ONE pass on expand and
  /// rendered lazily (a horizontal `ListView.builder` only builds the pills near
  /// the viewport). Stable across refreshes (no reshuffle); consumed/ineligible
  /// entries are pruned, and the set is re-picked only when empty. See
  /// [_refreshSuggestions].
  ...
  /// Upper bound on how many suggestions are materialised. Caps the weighted
  /// pick to O(cap·n) instead of the O(n²) a full-pool pick costs, while still
  /// offering far more than fits a focus screen. (Rendered lazily, so the cap is
  /// about the *pick* cost, not render.)
  static const int _suggestionCap = 40;
  ...
  ///  - Picks up to [_suggestionCap] eligible leaves in ONE weighted pass on
  ///    expand (weighted order, best first) and renders them lazily. Capping the
  ///    pick keeps it O(cap·n) rather than the O(n²) a full-pool pick costs.
  ///  - Order is stable: consumed/ineligible entries (in Today's 5, done today,
  ///    or now blocked / no longer a leaf) are pruned on each refresh, and the
  ///    set is re-picked only when empty — never reshuffled underfoot.
  Future<void> _refreshSuggestions() async {
    if (!_suggestionsExpanded || _todaysTasks.length >= maxPins) {
      if (_suggestions.isNotEmpty) _suggestions = [];
      return;
    }
    // Callers reach here after awaiting DB work (e.g. _loadOtherDoneToday) with
    // no mounted check in between — bail if the screen was disposed in that gap
    // so we don't read a deactivated BuildContext.
    if (!mounted) return;
    final provider = context.read<TaskProvider>();
    ...
    // Keep the existing (stable) order; only re-pick when nothing is left.
    ...
    // Cap the pick so a large tree never triggers a full-pool O(n²) pick; the
    // strip renders these lazily and the user scrolls through them.
    _suggestions = provider.pickWeightedN(
      pool,
      pool.length < _suggestionCap ? pool.length : _suggestionCap,
    ...
    return SizedBox(
      ...
      child: _fadeRightEdge(
        child: ListView.builder(
          ...
          // Lazily builds only the columns near the viewport; scrolls sideways
          // through the (capped) materialised set.
