...
import 'package:flutter/foundation.dart'
    show kDebugMode, setEquals, visibleForTesting;
...
class TodaysFiveScreenState extends State<TodaysFiveScreen>
  ...
  /// user chooses whether to accept one. Materialised **incrementally**: the
  /// first [_suggestionBatch] are weighted-picked on expand and more batches are
  /// appended (from the remaining eligible pool) as the strip is scrolled — so a
  /// huge task tree never pays for a single O(n²) `pickWeightedN` up front.
  /// Stable across refreshes (no reshuffle); consumed/ineligible entries are
  /// pruned. See [_refreshSuggestions] / [_loadMoreSuggestions].
  ...
  /// How many suggestions to weighted-pick per batch (initial + each scroll
  /// load-more). Bounds the up-front `pickWeightedN` cost to O(batch·n).
  static const int _suggestionBatch = 24;
  /// Guards re-entrant scroll-triggered batch loads.
  bool _loadingMoreSuggestions = false;
  /// True once a batch pick returned fewer than [_suggestionBatch] — the pool is
  /// drained, so stop triggering further load-more.
  bool _suggestionsExhausted = false;
  /// Drives the horizontal suggestion strip; its listener pages in more batches
  ...
  /// Pages in another suggestion batch when the strip is scrolled near its end.
  void _onSuggestionsScroll() {
    if (_loadingMoreSuggestions || _suggestionsExhausted) return;
    final pos = _suggestionsScrollController.position;
  ...
  ///  - Materialises only the FIRST [_suggestionBatch] on expand (weighted
  ///    order, best first); the strip pages in further batches as it's scrolled
  ///    ([_loadMoreSuggestions]). Order is stable: consumed/ineligible entries
  ///    are pruned on each refresh, and the batch is (re)picked only when the
  ///    list is empty (first expand, or after everything was accepted/dismissed)
  ///    — never reshuffled underfoot.
  Future<void> _refreshSuggestions() async {
    ...
    // Nothing to show → materialise the FIRST batch only. Further batches page
    // in on scroll (see [_loadMoreSuggestions]); weighted-picking the whole pool
    // at once is O(n²) in pickWeightedN and can freeze a large tree.
    _suggestionsExhausted = false;
    final picked = await _pickSuggestionBatch(provider, _suggestionBatch);
    _suggestions = picked;
    _suggestionsExhausted = picked.length < _suggestionBatch;
  }

  /// Weighted-picks up to [count] fresh suggestions from the currently eligible
  /// pool (leaf, unblocked, not pinned/done/dismissed, and not already in
  /// [_suggestions]). Capping the pick at [count] keeps each call O(count·n)
  /// instead of the O(n²) a full-pool pick would cost.
  Future<List<Task>> _pickSuggestionBatch(
      TaskProvider provider, int count) async {
    final inTodaysFive = _todaysTasks.map((t) => t.id).toSet();
    final doneToday = _otherDoneToday.map((t) => t.id).toSet();
    final allLeaves = await provider.getAllLeafTasks();
    final leafIds = allLeaves.map((t) => t.id!).toList();
  ...
  /// Appends the next weighted batch of suggestions — triggered when the strip
  /// is scrolled near its end ([_onSuggestionsScroll]). Guarded against
  /// re-entrancy and stops once the pool is drained ([_suggestionsExhausted]).
  Future<void> _loadMoreSuggestions() async {
    if (_loadingMoreSuggestions ||
        _suggestionsExhausted ||
        !_suggestionsExpanded ||
        _todaysTasks.length >= maxPins) {
      return;
    }
    _loadingMoreSuggestions = true;
    try {
      final provider = context.read<TaskProvider>();
      final picked = await _pickSuggestionBatch(provider, _suggestionBatch);
      if (!mounted) return;
      if (picked.length < _suggestionBatch) _suggestionsExhausted = true;
      if (picked.isNotEmpty) {
        setState(() => _suggestions = [..._suggestions, ...picked]);
      }
    } finally {
      _loadingMoreSuggestions = false;
    }
  }

  /// Test hook: number of suggestions currently materialised (a batch grows on
  /// scroll rather than the whole pool up front).
  @visibleForTesting
  int get materializedSuggestionCount => _suggestions.length;

  /// Test hook: force a scroll-triggered batch load without driving the
  /// ScrollController.
  @visibleForTesting
  Future<void> loadMoreSuggestionsForTest() => _loadMoreSuggestions();
    ...
    return SizedBox(
      ...
      child: _fadeRightEdge(
        child: ListView.builder(
          ...
          // strip is scrolled sideways. The controller's listener also pages in
          // the next weighted batch as the end nears (see _onSuggestionsScroll).
