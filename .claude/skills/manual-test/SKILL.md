...
4. Generate a numbered checklist of manual tests. Every UI element referenced MUST come from step 3's reading — never from memory or assumption. Format rules:
   ...
   - **Run a state-trace pass before output (mandatory, do NOT skip).** Build a mental table of every fixture/created task, then walk the steps in order and update each task's structural state after every step. The mutations that change structural state are not only the obvious "add a subtask" — they include **pinning/unpinning, completing/uncompleting, and (the one most often missed) filing / triaging / re-parenting one task UNDER another, which makes the *target* a non-leaf.** Example of the trap: a step files "Triage two" under "Tiles" (so "Tiles" is now a non-leaf), then a later step says "pick the leaf `Tiles`" — broken. For every step that names a task with a structural precondition (leaf, non-leaf, pinned, unpinned, in-inbox, empty…), confirm the table shows that exact state at that point; if it doesn't, swap in a fresh dedicated fixture. This trace is the single highest-value defense against the most common checklist defect.
