...
- **Present tests in batches by section.** Don't dump all test cases at once — show one section at a time (e.g. "Today's 5" tests first, then "All Tasks" after the user reports results). This prevents the list from feeling overwhelming and lets the user focus. **This applies to the caller too** — when presenting the agent's results, respect the batch boundaries rather than flattening everything into one list.
- **Snackbar undo tests are time-sensitive.** The undo snackbar only lasts 5 seconds. Don't write test cases that assume the user can tap Undo at leisure. If an undo test is needed, warn the user to act quickly, or suggest an alternative path (e.g. unarchive + re-add dependency manually) as a fallback if they miss the snackbar.
