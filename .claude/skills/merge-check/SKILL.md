...
1a. **Check whether the Claude review workflow is even enabled** before relying on it. Run `gh workflow list --all | grep -i claude`. If `Claude Code Review` shows `disabled_manually` (or the `claude-review` check never appears in `gh pr checks` for this PR), then the **Claude** AI review is **OFF**: do NOT wait for a `claude-review` check. Report clearly that "Claude AI review is disabled — this PR will not get a Claude review" so the user can decide whether that's acceptable.
    - **CRITICAL — Codex reviews are NOT visible from GitHub in this repo (verified 2026-07-26 on PR #80, and #79 shows the same).** Codex posts its results to **Codex Cloud** (chatgpt.com/codex), not to the GitHub API: `issues/{n}/comments`, `pulls/{n}/comments`, `pulls/{n}/reviews`, the issue timeline, check-runs and commit statuses are **all empty** even when Codex has reviewed and finished. Therefore:
      - **Empty GitHub endpoints prove NOTHING about Codex.** Never conclude "Codex didn't review", "no automated review", or "no issues found" from them — that inference is invalid and was made wrongly on #80.
      - **ASK THE USER to check Codex Cloud** for the review result, rather than polling GitHub and reporting silence. They can see whether Codex found issues; you cannot.
      - Past Codex findings (the P2s on #77/#78) reached the repo only because the **user relayed them** from Codex Cloud. Expect that flow.
      - Keep polling the GitHub endpoints anyway — but only as a cheap catch for *other* reviewers (a human, or `claude[bot]` if ever re-enabled). Do not attribute the outcome to Codex.
