...
1a. **Check whether the Claude review workflow is even enabled** before relying on it. Run `gh workflow list --all | grep -i claude`. If `Claude Code Review` shows `disabled_manually` (or the `claude-review` check never appears in `gh pr checks` for this PR), then the **Claude** AI review is **OFF**: do NOT wait for a `claude-review` check. Report clearly that "Claude AI review is disabled — this PR will not get a Claude review" so the user can decide whether that's acceptable.
    - **CRITICAL — Codex comments on GitHub ONLY when it finds issues (user-confirmed 2026-07-26).** So GitHub tells you about findings, but **silence is ambiguous**:
      - **Codex found issues** → it posts comments on the PR. Polling the comment endpoints IS worthwhile and will catch them. Address them per the rules below.
      - **Codex found NO issues** → it posts **nothing** on GitHub. The clean verdict is visible only on **Codex Cloud** — **chatgpt.com/codex/cloud** (note the `/cloud` suffix; plain `chatgpt.com/codex` is not the review list).
      - **Codex still running, or never triggered** → also nothing on GitHub.
      - Therefore **empty endpoints do NOT mean "reviewed and clean", and do NOT mean "never reviewed"** — the three cases are indistinguishable from GitHub. Never assert either one from silence. (This was gotten wrong on PR #80, where empty endpoints were reported as "no automated review at all" when Codex had in fact reviewed cleanly.)
      - **On silence, ASK THE USER to check Codex Cloud** — they can see both whether the review is still running and whether it reported no issues. Only they can disambiguate; do not guess, and do not present silence as a verdict either way.
      - Note check-runs and commit statuses carry no Codex entry in either case, so there is no check to wait on.
