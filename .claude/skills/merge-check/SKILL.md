...
1a. **Check whether the Claude review workflow is even enabled** before relying on it. Run `gh workflow list --all | grep -i claude`. If `Claude Code Review` shows `disabled_manually` (or the `claude-review` check never appears in `gh pr checks` for this PR), then the AI review is **OFF**: do NOT wait for a `claude-review` check, and **skip the Review comments agent entirely** (there will be no review and no comments). Report clearly that "AI review is disabled — this PR will not get an automated review" so the user can decide whether that's acceptable. The other reviewer (`codex`) is independent — still check for it if its check is present.
2. Launch **two background agents in parallel** (or just the CI agent if AI review is disabled per 1a):
...
8. If **no comments arrived at all** after polling, tell the user and **wait for explicit confirmation** before merging. Do NOT run the merge command until the user says to proceed — they may want to check manually first.
   - **If AI review is disabled (step 1a) or the logs show it errored without running, there is NO automated review of this PR.** State that plainly (don't imply it was reviewed and clean) and wait for explicit confirmation before merging, since only CI + the user's own testing have vetted the change.
