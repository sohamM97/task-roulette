...
1. **Test generation agent** — runs `/add-auto-tests` with the `last` argument in a **worktree** (`isolation: "worktree"`) to prevent unintended production code changes. **Important:** Tell the agent to look at the current branch's changes (both committed and uncommitted) compared to `main`, NOT other branches.
